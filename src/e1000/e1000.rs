...
impl<'a, K: KernelFunc> E1000Device<'a, K> {
    ...
    pub fn e1000_init(&mut self) {
        ...
        let tx_ring_len = TX_RING_SIZE * size_of::<TxDesc>();
        let tdba = self.tx_ring_dma;
        self.regs[E1000_TDLEN].write(tx_ring_len as u32);
        self.regs[E1000_TDBAH].write((tdba >> 32) as u32);
        ...
        let rx_ring_len = RX_RING_SIZE * size_of::<RxDesc>();
        let rdba = self.rx_ring_dma;
        self.regs[E1000_RDLEN].write(rx_ring_len as u32);
        self.regs[E1000_RDBAH].write((rdba >> 32) as u32);
        self.regs[E1000_RDBAL].write((rdba & 0x00000000ffffffff) as u32); 
